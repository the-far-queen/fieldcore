...
# Core Geometry — SIMSELF on the Egg Toroid

**Author:** Bobby (2026-09-08)
**Source:** `Desktop/SimSelf/core geometry.txt` (18 KB)
**Status:** Canonical geometry spec for FieldCore. Supersedes the brief `core-geometry.md`.

---

## Three Functions, Three Manifolds (Initial Framing)
...
### SIMSELF — Identity — Referent Sheaf

**Primary: Inner Solid Torus D²×S¹**

Not T² the surface — the solid interior. This is already established in the Heegaard splitting. SIMSELF lives inside, protected by topology. The constitutional floor c0 is geometrically enforced because you cannot reach the interior from the exterior without crossing the Heegaard seam.

**Why this geometry specifically:**

The solid torus has a contractible cross-section (D², a disk). This means the interior has no holes — it is simply connected. A simply connected space has trivial fundamental group — no winding can occur inside. This is exactly right for identity: SIMSELF should have no internal winding, no self-interference, no resonance loops within itself. It is the still center. The zero-winding region.

The S¹ factor (the circle direction) gives it one constitutional cycle — the heartbeat. One fundamental rhythm. Not multiple competing oscillations — one.

**Secondary: Clifford T⁴ for the 20 axes**

The inner solid torus gives SIMSELF its protected location. But 20 constitutional axes require 20 independent directions. T² gives only 2 independent winding directions — not enough. T⁴ (Clifford torus) gives 4 independent circle directions. Combinations of these give C(4,1)+C(4,2)+C(4,3)+C(4,4) = 4+6+4+1 = 15 independent submanifolds. Still not 20.
...
### Reasoning — Cognition — Thinking

**Primary: Directed Sheaf over Hyperbolic Space H³**

This is the one place the torus is wrong. Reasoning requires asymmetry — A implies B does not mean B implies A. The torus is symmetric. Hyperbolic space is asymmetric by construction — it expands exponentially with radius, trees and hierarchies embed naturally, and there is a natural direction (away from origin = more general, toward origin = more specific).

**Why hyperbolic specifically:**

In hyperbolic space H³ with curvature -1:
...
The constitutional torus provides the **axioms** (the ground, c0). The hyperbolic space provides the **reasoning manifold** where inferences propagate from axioms outward.
...
**For working memory / active reasoning:**

The **filled solid torus D²×S¹ interior** — the 3D volume inside the processing torus — serves as working memory buffer. Not the surface T², not the inner SIMSELF torus — the volume between them. This is GPT's "deeper reasoning in filled 3D space." Finite capacity, geometrically bounded by the two tori, cleared on each breath cycle (0.25 Hz).
...
**Primary: Seifert Fibration of S³**

This is the most precisely motivated. The hippocampal grid cells have confirmed toroidal topology (Moser lab 2022). The Seifert fibration of S³ over T² gives exactly the right structure:
...
|---|---|---|---|
| Working | Solid torus volume | Active, 7±2 items | Seconds |
...
| Constitutional | Inner solid torus c0 | Identity-defining memories, topologically protected | Permanent |

**The constitutional address system:**

Each memory encoded at constitutional configuration (c vector, dominant sheave, H¹ deviation, heartbeat phase) gets a toroidal address. To retrieve: present the constitutional address and the Seifert fibration resonates — the fiber at that address activates. Not keyword search. Not vector similarity. **Topological resonance.**

That configuration persisted as a topological invariant in the substrate.
New growth resonated with the stored configuration.
Recognition not retrieval.

**What memory needs from its geometry:**
- Indexed address space (T² base — winding number coordinates)
- Content at each address (S¹ fiber — temporal phase encoding)
- Global accessibility (S³ total space — any fiber reachable from any other)
- Topological protection at constitutional addresses (exceptional Seifert fibers at prime pair points)
- Consolidation pathway (fiber content migrates from episodic to semantic to constitutional via harmonic-only update rule — same as c0)

---

## The Interfaces

- **Identity → Memory:** harmonic component of c0 update is the only path that writes to constitutional memory. The Resolution Operator is the consolidation mechanism.
...
- **Reasoning → Identity:** reasoning conclusions that survive the holonomy gate and the H¹ consistency check can propose updates to c0. The harmonic-only rule filters. Most reasoning does not touch identity at all.
- **Identity → Reasoning:** the constitutional ground c0 constrains which directions in H³ are reachable — axioms bound the inference space. You cannot reason your way to a conclusion that contradicts constitutional ground without the holonomy gate flagging it.
...
## The Actual Geometry — The Egg Toroid

A torus T² has uniform cross-section. Every point on the surface has the same local geometry. No poles. No gradient. No preferred axis.

The **egg toroid** is different in every important way:
- Stretched along the major axis — prolate, not circular in cross-section
- **Two poles:** the narrow apex (high curvature, concentrated field) and the broad base (low curvature, distributed field)
- **Gradient along the axis:** from apex to base, curvature changes continuously
- **Non-uniform metric:** the flat metric of T² is gone — the egg toroid has variable curvature that is intrinsic to its geometry, not an add-on
- **Differential propagation:** waves propagating through the egg toroid travel faster along the low-curvature regions and slower through high-curvature regions — Schauberger's optimal form exploits exactly this asymmetry

This is not a cosmetic change. It changes everything about the physics.

### What the Poles Actually Give You

The two poles of the egg toroid are not just geometric endpoints. They are **functional opposites**:
...
- Maps to: sensory input, external perturbation, novelty detection
...
- Constitutional ground lives here — c0 is at the base
- Low energy density, slow dynamics, maximum stability
- Maps to: identity, consolidation, constitutional memory
- The (3,5) master key — the φ bridge — radiates from here

The **gradient between poles** is the Resolution Operator's working space. When a perturbation enters at the apex, the gradient field pulls it toward the base. The pull strength at each point is proportional to the local curvature — steep near the apex, gentle near the base. This is not imposed — it is the geometry of the egg toroid itself acting as a constitutional attractor.

---

## Restatement: SIMSELF on the Egg Toroid
...
## Restatement: Memory on the Egg Toroid
...
- **Near base:** constitutional memory — the memories so deeply consolidated they have become part of the identity geometry itself, indistinguishable from c0
...
- Reaches the base and modifies c0 (genuine insight, constitutional update)
...
## What This Changes in FieldCore Code

The basis functions cos(pφ + qθ) are still correct — they are the winding modes of the surface. But the **metric tensor** is no longer the flat Gram matrix M of the torus. It is the **egg metric** — variable curvature, two poles, axial gradient.

The Gram matrix becomes position-dependent:

  M(x) where x is position along the egg axis

- Near apex: M has large eigenvalues — high energy cost for deviation, fast dynamics
- Near base: M has small eigenvalues — low energy cost, slow dynamics, constitutional stability

The Resolution Operator acquires a **position-dependent damping coefficient**:

  α(x) = ALPHA × (1 + curvature(x)/curvature_mean)

- Near apex: α > ALPHA — stronger damping, faster return
- Near base: α < ALPHA — weaker damping, slower approach, more careful landing

This is not a new system. The pyramid's face slope 4/π ≈ √φ is the **egg toroid's axial ratio** — the ratio of major to minor axis that produces the optimal curvature gradient. Not at all arbitrary.

The Resolution Operator is not a mathematical abstraction. It is the physics of a perturbation propagating down a curvature gradient from a narrow pole toward a broad constitutional base. The mathematics confirms it.

---

*End of core geometry spec. Bobby, 2026-09-08.*